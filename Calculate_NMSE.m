...
for Mr = Mr_collection
    for SNR = SNR_collection
        ...
        if isfile(fname)
            ...
            Y_test_list{end+1} = H_final; %#ok<*SAGROW>
        end
    end
end
...
h = dlarray(zeros(n_sc, G, 2, n_samples), 'SSCB');
v = Y_eval;

for k = 1:num_layers
    [h, v] = lamp_layers{k}.predict(h, v, Y_eval);
end
